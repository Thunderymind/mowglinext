...
| `connector_max_headland_passes` (issue #497) | 0 | injected **unclamped** → `coverage_server.connector_max_headland_passes`; `coverage_planning.cpp` clamps to `[0, n_rings]` and moves `connector_clearance_boundary` from ring 0's centerline to ring `(n_rings − value)`'s. 0 = unlimited (ring 0, unchanged default). No effect when `num_headland_passes < 0` | Mowing | dynamic |
