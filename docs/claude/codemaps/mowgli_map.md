...
| `dock_set_cog_max_age_s`, `dock_set_cog_max_sigma_rad`, `dock_set_yaw_bias_max_rad` | 30.0, 0.15, 0.15 | L311-316 | issue #446 gate (2b), `on_set_docking_point`'s `use_gps_position` branch (`area_manager.cpp` ~L823-896): cross-checks the fused yaw against `/imu/cog_heading` before averaging `/gps/pose_cov` samples — gate (3) only proves the fused yaw was STABLE, not ACCURATE, and a settled-wrong magnetometer lock silently biases the lever-arm-corrected GPS position. Fails closed (rejects the service call) if no fresh/confident COG sample exists or the two yaws disagree beyond `dock_set_yaw_bias_max_rad`. `last_cog_yaw_rad_`/`_variance_rad2_`/`_time_` populated by the `cog_heading_sub_` lambda, `map_server_node.cpp` L317-327 |
