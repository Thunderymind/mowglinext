...
Settings updates: `components/settings/HostUpdaterPanel.tsx` and `hooks/useHostUpdater.ts` implement Simple/matched updates, Advanced published versions or explicit custom images for every installed managed service, separate trust/download and installation acknowledgements, standard-deployment/custom-mix identity, collapsed preferences/history/diagnostics, cached runtime identity/health and source selection. Browser freshness compares a build ID embedded in JavaScript with `web-build.json` generated by Vite, separately from the Go backend revision (`hooks/useInstalledVersions.ts`). The Simple view shows installed and available releases once, with review before the running component list. `AppShell.tsx` renders verified running release/version summaries in the desktop rail and mobile More sheet (custom/unknown identities remain explicit). Fixture screenshots and behavior coverage: `tests/e2e/host-updater.spec.ts`.
...
| Fusion-graph diagnostics panel | `pages/DiagnosticsPage.tsx` `sectionFusionGraph` (keys parsed right above it), service buttons `callFusionService` (`fusion_graph_save` / `fusion_graph_clear` / `fusion_graph_clear_lidar_map`, the last two behind a confirm); feed `hooks/useFusionGraphDiagnostics.ts`. The "LiDAR anchor" tile group (state / map cells / verdict + hit ratio / factors / accepted vs rejected / reseeds / odom rebases) and the "Clear LiDAR map" button render only when `lidar_anchor_state` is in the diagnostics — derivation is the pure `utils/lidarAnchor.ts` `deriveLidarAnchor` (unit-tested) |
| LiDAR localizer toggles (settings) | `components/settings/LocalizationSection.tsx`: `use_lidar_map_anchor` and dependent `lidar_anchor_shadow_mode`; both write through `POST /settings/yaml` |
...
| `src/types/irrisense.ts`, `src/types/remoteAccess.ts`, `src/types/mapbox-gl-draw.d.ts`, `src/global.d.ts`, `src/vite-env.d.ts` | 76/60/72/12/1 | IrriSense DTOs; remote-access DTOs + `remoteAccessVerdict`; ambient typings |
...
| Section components: `HardwareSection` `DriveMotorSection` `NtripSection` `PositioningSection` `SensorsSection` `LocalizationSection` `MowingSection` `DockingSection` `BatterySection` `SafetySection` `ObstaclesSection` `NavigationSection` `RainSection` `LedsSection` `IrriSenseSection` `RemoteAccessSection` `AdvancedSection` `DisplayModeSection` `LogTimeZoneSection` | 64–964 | One per `SettingsSection` id, except `appearance`, which renders `DisplayModeSection` + `LogTimeZoneSection` (18 components for 17 ids); `DriveMotorSection` (964) also hosts PID/FF tuning runs |
...
| `RemoteAccessConnectionCard.tsx` / `RemoteAccessStatusCard.tsx` | ~100/~150 | Tailscale node name / write-only auth key / HTTPS toggle / image; status card with login link, reachable URLs, retry and logout (`hooks/useRemoteAccessStatus.ts` polls `/remote-access/status` every 5 s) |
...
- **New backend REST route or handler signature** → `gui/docs/swagger.json` → `yarn generate:api` → `src/api/Api.ts`. Hand-written `guiApi.request({path: …})` calls (`/params`, `/settings/yaml/defaults`, `/tools/*`, `/irrisense/*`, `/remote-access/*`) bypass the generated client and must be updated by hand.
...
- The dashboard mini-map (`MowgliNextPage`) and MapPage (`useMapStreams`) both rasterize the same `OccupancyGrid` through `utils/mowProgress.ts` (the row flip + canvas pass is `utils/occupancyGrid.ts`, shared with `utils/lidarMap.ts`); the grid↔OccupancyGrid axis convention is CLAUDE.md Invariant 14 — mis-sized rasters show as a 90°-rotated overlay.
