...
TEST(SlipPivot, TruePivotStillTracksWithoutPhantomTranslation)
{
  ...
  const double sigma_xy = std::sqrt(std::max(snap->covariance(0, 0),
                                              snap->covariance(1, 1)));
  ...
  EXPECT_LT(drift_m, 0.05);
  EXPECT_LT(sigma_xy, 0.15);
  EXPECT_NEAR(yaw, kWz * kDt * kTicks, 0.1 * kWz * kDt * kTicks);
}

// ─────────────────────────────────────────────────────────────────────
// Wheel xy uncertainty must be continuous across every curve radius.  At the
// same forward distance, changing yaw rate may not change its sigma; a true
// pivot (zero distance) receives only the small stationary floor.
// ─────────────────────────────────────────────────────────────────────
TEST(SlipPivot, WheelNoiseDependsOnDistanceNotCurveRadius)
{
  auto gp = MakeParams();
  gp.node_period_s = 0.04;  // 25 Hz, as on the rover
  ...
  constexpr double kClockStep = 0.041;  // strictly clears the 40 ms node gate
  constexpr double kVx = 0.167;
  constexpr double kExpected = 0.05 * (kVx * kDt / 0.008);
  double elapsed = 0.0;
  for (const double wz : {0.05, 0.441, 1.5})
  {
    gm.AddWheelTwist(kVx, 0.0, wz, kDt);
    gm.AddGyroDelta(wz, kDt);
    elapsed += kClockStep;
    ASSERT_TRUE(gm.Tick(elapsed).has_value());
    EXPECT_NEAR(gm.Stats().wheel_sigma_x_eff, kExpected, 1.0e-12);
  }

  gm.AddWheelTwist(0.0, 0.0, 1.5, kDt);
  gm.AddGyroDelta(1.5, kDt);
  elapsed += kClockStep;
  ASSERT_TRUE(gm.Tick(elapsed).has_value());
  EXPECT_NEAR(gm.Stats().wheel_sigma_x_eff, 0.005, 1.0e-12);
}
