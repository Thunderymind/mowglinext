...
void FollowStrip::updateProgress(const std::shared_ptr<BTContext>& ctx)
{
  ...
  // A closed boundary returns to its start.  Searching the entire small loop
  // lets a robot standing at the start match its final pose and falsely claim
  // near-100% progress.  A bounded, physically local look-ahead preserves
  // monotonic tracking while requiring traversal of the loop.
  constexpr std::size_t kMaxForwardSearch = 64;
  const bool closed_loop = poses.size() >= 3 &&
      std::hypot(poses.front().pose.position.x - poses.back().pose.position.x,
                 poses.front().pose.position.y - poses.back().pose.position.y) < 0.20;
  const std::size_t closed_loop_cap = closed_loop
      ? std::max<std::size_t>(1, poses.size() / 4)
      : kMaxForwardSearch;
  const std::size_t search_window = std::min(kMaxForwardSearch, closed_loop_cap);
  const std::size_t end = std::min(poses.size(), path_progress_idx_ + search_window);
...
bool FollowStrip::sendFollowGoal(const std::shared_ptr<BTContext>& ctx)
{
  ...
  // Nav2's goal checker considers a closed loop complete when the robot is
  // already near its terminal pose (which equals its start).  Extend a closed
  // perimeter by a short repeat of its opening arc, making the terminal goal
  // unambiguously ahead of the robot while retaining the full boundary.
  if (goal.path.poses.size() >= 3)
  {
    const auto& first = goal.path.poses.front().pose.position;
    const auto& last = goal.path.poses.back().pose.position;
    if (std::hypot(first.x - last.x, first.y - last.y) < 0.20)
    {
      constexpr double kClosedLoopGoalSeparationM = 0.80;
      for (std::size_t i = 1; i < swaths_[swath_idx_].poses.size(); ++i)
      {
        const auto& p = swaths_[swath_idx_].poses[i];
        goal.path.poses.push_back(p);
        const auto& q = p.pose.position;
        if (std::hypot(q.x - first.x, q.y - first.y) >= kClosedLoopGoalSeparationM)
        {
          break;
        }
      }
      RCLCPP_INFO(ctx->node->get_logger(),
                  "FollowStrip: extended closed segment %zu with a seam overlap; terminal goal is ahead of start",
                  swath_idx_ + 1);
    }
  }
