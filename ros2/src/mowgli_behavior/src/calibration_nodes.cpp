...
BT::NodeStatus SeedYawFromMotion::onStart()
{
  ...
  if (!ctx->gps_is_fixed)
  {
    if (ctx->gps_fix_type >= 3u)
    {
      // RTK Float is sufficient to start an area at reduced navigation
      // speed, but not to derive a reliable yaw from a one metre baseline.
      // Keep the dock/odometry heading instead of failing the entire mowing
      // session; COG/yaw calibration remains restricted to RTK Fixed.
      ctx->yaw_seeded_this_session = true;
      RCLCPP_INFO(ctx->node->get_logger(),
                  "SeedYawFromMotion: RTK Float at start (fix_type=%u); "
                  "keeping dock/wheel heading and continuing in degraded mode.",
                  ctx->gps_fix_type);
      return BT::NodeStatus::SUCCESS;
    }
    RCLCPP_WARN(ctx->node->get_logger(),
                "SeedYawFromMotion: no RTK Float-or-better fix at start "
                "(fix_type=%u), aborting seed.",
                ctx->gps_fix_type);
    return BT::NodeStatus::FAILURE;
  }
